...
export const POST: APIRoute = async (ctx) => {
  ...
  const result = await handleAddIdentity(dao, ctx.params.id!, body.kind, body.identityKey, new Date());
...
export const DELETE: APIRoute = async (ctx) => {
  ...
  const body = (await ctx.request.json().catch(() => ({}))) as { kind?: unknown; identityKey?: unknown };
  const dao = createPlayerMutationsDao(createDb(env.DB));
  const result = await handleRemoveIdentity(dao, ctx.params.id!, body.kind, body.identityKey);
  if (!result.ok) return Response.json({ ok: false, error: result.error }, { status: result.status });
  return Response.json(result);
};
